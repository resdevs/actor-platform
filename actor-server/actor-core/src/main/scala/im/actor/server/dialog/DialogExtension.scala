...
final class DialogExtensionImpl(system: ActorSystem) extends DialogExtension with PeersImplicits {
  ...
  // this is message that sent on behalf of user `senderUserId`,
  // and was initiated by server, not client
  // отправка сервисного сообщения от имени пользователя будет состоять из двух шагов:
  // непосредственно отправка сообщения без указания authId
  // получение seqState для данного пользователя с данным authId
  //
  /**
   *
   * Send message on behalf of `senderUserId`, and return SeqStateDate
   * for `senderAuthId`. The main difference here, is that inside,
   * `UpdateMessage` pushed to all sender's devices(in contrary, usually we push `UpdateMessageSent` to `senderAuthId` device)
  ...
  def sendServerMessage(
    ...
    randomId:     RandomId,
    message:      ApiMessage,
    deliveryTag:  Option[String] = None // tag that we will use to optimize sequence TODO: none by default
  ): Future[SeqStateDate] = {
    val mPeer = peer.asModel
